	...
	<main>
		...
		<Section title="What we measure">
			...
			<dl class="metric-list">
				<div class="metric-item">
					<dt>Energy (Joules)</dt>
					<dd>
						Total electrical energy consumed by the GPU during the inference run. This is the
						primary metric. Joules are the standard unit of energy: one joule equals one watt of
						power sustained for one second. We also report energy per output token (J/token), which
			...
			<dl class="config-list">
				...
				<div class="config-item">
					...
					<dd>
						...
						memory. <strong>SDPA</strong> (Scaled Dot-Product Attention, built into PyTorch 2.0+)
						uses memory-efficient kernels. <strong>Flash Attention v2</strong> reorders memory access
						to minimise GPU memory bandwidth, typically reducing energy by 15-20% relative to standard
						attention.
					</dd>
				</div>
			</dl>
		...
		<Section title="How we measure">
			...
			<ol class="method-steps">
				<li>
					<strong>Baseline subtraction.</strong> We measure idle GPU power before inference begins and
					subtract it from the total. This isolates the energy cost of the inference workload from background
					GPU activity (driver overhead, monitoring processes, etc.).
				</li>
				<li>
					<strong>Warmup phase excluded.</strong> The first inference pass is discarded. GPU resources
					(memory allocation, JIT compilation, cache warming) are initialised during warmup and would
					inflate energy readings if included.
				</li>
				<li>
					<strong>Multiple cycles averaged.</strong> We run each configuration multiple times and report
					aggregate statistics. This smooths measurement noise from OS scheduling, thermal variation,
					and GPU clock frequency fluctuations.
				</li>
			</ol>
			...
			<ExpandableDetail title="Technical detail: measurement protocol">
				...
				<p>
					...
					configuration, enabling deduplication and incremental dataset updates as new measurements are
					added.
				</p>
			</ExpandableDetail>
		</Section>
		...
		<Section>
			<ExpandableDetail title="Limitations and caveats">
				<ul>
					<li>
						<strong>Single GPU only.</strong> All measurements reflect single-GPU inference. Multi-GPU
						deployments have different energy profiles and communication overhead not captured here.
					</li>
					<li>
